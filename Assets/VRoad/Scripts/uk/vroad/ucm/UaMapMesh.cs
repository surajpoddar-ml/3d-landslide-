...
namespace uk.vroad.ucm
{
    ...
    public abstract class UaMapMesh : MonoBehaviour, LUAppState, LSimTimeSec
    {
        ...
        protected Material[] buildingMaterials;
        
        private Material aerialMaterial;
        protected Texture2D aerialTexCrop;
        ...
        private double redAndAmberTimeBeforeGreen = 0;
        ...
        protected GameObject NewGoMesh(string goMeshName, Material templateMat, Color color)
        {
            // Color can not be null
            if (templateMat == null) templateMat = GetDummyMaterial(goMeshName);

            return NewGoMesh(goMeshName, NewMaterial(goMeshName, templateMat, color));
        }
       
        protected GameObject NewGoMesh(string goMeshName, Material instanceMat)
        {
        ...
        protected virtual void Awake()
        {
            ...
            goMeshLanes = NewGoMesh(SA.MESH_LANES, lanes.junction);
            goMeshJunctions = NewGoMesh(SA.MESH_JUNCTIONS, lanes.junction);
            goMeshTurnArrows = NewGoMesh(SA.MESH_TURN_ARROWS, lanes.turnArrows);

            goMeshSignalBoxes = NewGoMesh(SA.MESH_SIGNAL_BOXES, signals.box);
            goMeshSignalArrows = NewGoMesh(SA.MESH_SIGNAL_ARROWS, signals.off); 
            goMeshSignalWalks = NewGoMesh(SA.MESH_SIGNAL_WALKS, decals.walkRed);

            goMeshBusStops = NewGoMesh(SA.MESH_BUS_STOPS, decals.busBay);
            goMeshParking = NewGoMesh(SA.MESH_PARKING, decals.parkingBay);
            goMeshDropOff = NewGoMesh(SA.MESH_DROP_OFF, decals.dropOffBay);

            goMeshFootpaths = NewGoMesh(SA.MESH_FOOTPATHS, materials.footpath);
            goMeshCrossings = NewGoMesh(SA.MESH_CROSSINGS, materials.crossingPlain);
            goMeshZebras = NewGoMesh(SA.MESH_ZEBRAS, materials.crossingZebra);
           
            goMeshSolidBuildings = NewGoMesh(SA.MESH_SOLID_BUILDINGS, materials.building);
            goMeshGlassBuildings = NewGoMesh(SA.MESH_DRIVE_THROUGHS, materials.buildingGlass);
            goMeshWater = NewGoMesh(SA.MESH_WATER, materials.water);
            
            // When layered terrain is used, the simple terrain is used for tunnel entrances only 
            goMeshTerrainSimple = NewGoMesh(SA.MESH_TERRAIN, materials.terrain);
            ...
            goMeshBridges = NewGoMesh(SA.MESH_BRIDGES, materials.bridge);
            goMeshTunnels = NewGoMesh(SA.MESH_TUNNELS, materials.tunnel);
            goMeshShoulders  = NewGoMesh(SA.MESH_SHOULDERS, materials.shoulder);
            goMeshEmbankments = NewGoMesh(SA.MESH_EMBANKMENTS, materials.embankment);
            goMeshBarriers  = NewGoMesh(SA.MESH_BARRIERS, materials.barrier);
            goMeshMedians  = NewGoMesh(SA.MESH_MEDIANS, lanes.medianMarking);
        ...
        /// <summary>
        /// This creates a single array of building materials, of a size to match the number of sub-meshes
        /// in the SolidBuildings GameObject. 
        /// </summary>
        protected virtual void SetupBuildingMaterials()
        {
        ...
        protected virtual void SetTerrainMaterial()
        {
            ...
            if (aerialMapZoomLevel > 0) // The slippy map zoom level: 13, 14, ...
            {
                ...
                aerialTexCrop.filterMode = FilterMode.Point;
        ...
        IEnumerator CreateMapMeshes(bool useMulti, bool useMixed)
        {
            ...
            if (useMulti) // one mesh for each object (each lane, junction, etc)
            {
                ...
                if (parameters.simulationMeshes)
                {
                    ...
                    yield return null;
                    Material[] signalColourMaterial = new Material[] {signals.off,};
                    CreateNamedMeshObjects(25, goMeshSignalArrows, shsma[1], signalColourMaterial, new int[0]);
                    yield return null;
                ...
                CreateSolidBuildings(60); yield return null; // overridden by PTR
                CreateGlassBuildings(64); yield return null; // overridden by PTR
            ...
            {
                CreateNamedMeshObjects(4, goMeshLanes, LanesByMarking(), laneMaterials, null); yield return null;
                CreateMesh(8, goMeshJunctions, JunctionSubMesh(junctionToUpper)); yield return null;
                CreateMesh(12, goMeshTurnArrows, TurnArrowSubMesh()); yield return null;
        ...
        protected virtual void CreateSolidBuildings(int progress)
        {
            if (VRoad.GotPro() && parameters.randomizeBuildings)
                CreateNamedMeshObjects(progress, goMeshSolidBuildings, SolidBuildingsRandomized(), buildingMaterials, null);
            else CreateMesh(progress, goMeshSolidBuildings, BuildingSubMesh(false));
        }

        protected virtual void CreateGlassBuildings(int progress)
        {
            CreateMesh(progress, goMeshGlassBuildings, BuildingSubMesh(true));
        }
        ...
        {
            UaStateHandler.MostRecentInstance.SetMeshCreationFinished();
        }
        ...
        public void TimeSec()
        {
            ...
            if (redAndAmberTimeBeforeGreen > 0) turnsJustChangedArray = App().Map().Turns();
            else
            {
        ...
        protected virtual void UpdateDisplay()
        {
            ...
            if (ta.Length > 0)
            {
                ...
                int nch = goMeshSignalArrows.transform.childCount;
                if (nch > 0)
                {
                    foreach (ITurn turn in ta)
                    {
                        ...
                        foreach (IStreme s in turn.GetStremes())
                        {
                            ...
                            if (gmi < 0 || gmi >= nch - 2) continue;

                            for (int ci = 0; ci < 3; ci++)
                            {
                                GameObject goArrow = goMeshSignalArrows.transform.GetChild(gmi + ci).gameObject;
                                MeshRenderer mr = goArrow.GetComponent<MeshRenderer>();
                                Material[] mrsma = mr.sharedMaterials;
                                if (mrsma.Length != 1) continue;

                                // If the index of this child GO matches the current turn colour index, then set it to
                                // the material for that colour, otherwise off.  
                                mrsma[0] = ci == sci ? sigmat[sci] : matOff;
                    ...
                    foreach (ITurn turn in ta)
                    {
                        ...
                        foreach (IStreme s in turn.GetStremes())
                        {
                            ...
                            for (int ci = 0; ci < 3; ci++)
                            {
                                ...
                                if (redAndAmberTimeBeforeGreen > 0) // Show red+amber before green
                                {
                                    if (ci == 1 && sci == 2) // if this is yellow lamp, and this turn is at red
                                    {
                                        IJunctionControl ijc = s.GetJunction().GetJunctionControl();
                                        IStage cs = ijc.GetCurrentStage();
                                        if (cs.GetRemainingGreen() < redAndAmberTimeBeforeGreen)
                                        {
                                            IStage ns = ijc.GetNextStage();
                                            ITurn[] gta = ns.GetTurnsOnSignal(Signal.Green);
                                            if (gta.Contains(turn)) mrsma[gmi + ci] = sigmat[ci];
                                        }
                                    }
                                } 
                            }
        ...
        public void CreateNamedMeshObjects(int progress, GameObject goMesh, NamedSubMesh[] nmoa)
        {
            CreateNamedMeshObjects(progress, goMesh, nmoa, null, null);
        }

        protected void CreateNamedMeshObjects(int progress, GameObject goMesh, NamedSubMesh[] nsma, Material[] ma, int[] mia)
        {
            ...
            for(int nsmi = 0; nsmi < nnsm; nsmi++)
            {
                ...
                else if (mia != null && mia.Length == 0 && ma.Length == 1)
                {
                    mr.sharedMaterials = new[] { ma[0], };
                }
                else
                {
                    int index = mia[nsmi];  
        ...
        protected IOutline[] SolidBuildings()
        {
            List<IOutline> oll = new List<IOutline>();

            IOutline[] outlines = App().Map().Outlines();

            foreach (IOutline ol in outlines)
            {
                if (ol.IsBuilding() && !ol.IsBuildingStraddlingRoad()) oll.Add(ol);
            }
        ...
        /// <summary>
        /// This creates an array of indices into the building materials array. This array of indices has
        /// a size equal to the number of buildings - there is one index for each building
        /// </summary>
        /// <param name="nb"></param>
        /// <returns></returns>
        protected int[] SolidBuildingMaterialIndexArray(int nb)
        {
            int[] bmia = new int[nb];
            if (! VRoad.GotPro()) 
            {
            ...
            foreach (IOutline ol in outlines)
            {
                if (ol.IsBuilding() && !ol.IsBuildingStraddlingRoad())
                {
                    ...
                    int bmi = BuildingMaterialIndex(bti);
        ...
        protected int BuildingMaterialIndex(int bti)
        {
        ...
        NamedSubMesh[] SolidBuildingsRandomized()
        {
            ...
            int nbt = buildingMaterials.Length;
            if (nbt == 0) nbt = 1;
        ...
        NamedSubMesh[][] SignalHeadNamedSubMeshArray()
        {
            ...
            foreach (IStreme s in stremes)
            {
                ...
                if (shtma != null && shtma.Length == 4)
                {
                    ...
                    smla.Add(TriangleNamedSubMesh(s, shtma[1], 0, 0, "G"));
                    smla.Add(TriangleNamedSubMesh(s, shtma[2], 0, 0, "Y"));
                    smla.Add(TriangleNamedSubMesh(s, shtma[3], 0, 0, "R"));
                }
            }
        ...
        protected NamedSubMesh[] BuildingNamedSubMeshArray(bool isGlass)
        {
            ...
            foreach (IOutline ol in outlines)
            {
                ...
                {
                    TriMesh walls = ol.WallsTriMesh();
                    TriMesh flatRoof = ol.UpperSurfaceTriMesh();

                    nmol.Add(TriangleNamedSubMesh(ol, TriMesh.Combine(new[] {walls, flatRoof,})));
                }
            }
        ...
        public static NamedSubMesh TriangleNamedSubMesh(object mapObject, TriMesh triMesh, float dh = 0, int mati = 0, string suffix = null)
        {
            return (NamedSubMesh) TriangleSubMesh(mapObject, triMesh, dh, mati, suffix);
        }

        public static SubMesh TriangleSubMesh(object mapObject, TriMesh triMesh, float dh = 0, int mati = 0, string suffix = null)
        {
            ...
            try
            {
                ...
                return new NamedSubMesh(mapObject, va, tvia, na, uva, mati, suffix);
        ...
        protected Material RandomWallMaterial(IOutline ol, Material[] wallMaterials)
        {
            int nwm = wallMaterials == null ? 0 : wallMaterials.Length;
            if (nwm == 1) return wallMaterials[0];
            if (nwm > 1) return wallMaterials[Rng.NextInt(Rng.Vein.BUILD_MISC, nwm)];

            return RandomBuildingMaterial(ol);
        }

        protected Material RandomRoofMaterial(IOutline ol, Material[] roofMaterials)
        {
            int nrm = roofMaterials == null ? 0 : roofMaterials.Length;
            if (nrm == 1) return roofMaterials[0];
            if (nrm > 1) return roofMaterials[Rng.NextInt(Rng.Vein.BUILD_MISC, nrm)];

            return RandomBuildingMaterial(ol);
        }
        private Material RandomBuildingMaterial(IOutline ol)
        {
            int bti = ol.BuildingType(); // { 0=residential, 1=commerical, 2=industrial, 3=storage, }
            if (bti < 0) bti = Rng.NextInt(Rng.Vein.BUILD_MISC, 4);
            int bmi = BuildingMaterialIndex(bti);
            int nbt = buildingMaterials.Length;
            if (bmi < 0 || bmi >= nbt) bmi = 0;

            return buildingMaterials[bmi];
        }

        protected void SetupMeshColliders()
        {
        ...
        protected void SetupMeshCollider(GameObject goMesh)
        {
