...
namespace uk.vroad.Editor
{
    ...
    public abstract class VRoadEditorWindow : EditorWindow, LSceneSave
    {
        ...
        private static void NoEditTextFieldL1F2(string label, string value1, string value2, 
            float labelWidth, float columnWidth)
        {
            GUILayout.BeginHorizontal();
            {
                EditorGUILayout.LabelField(label, GUILayout.MaxWidth(labelWidth - 1));

                float textfieldWidth = (columnWidth - labelWidth) / 2;
                NoEditTextField(value1, textfieldWidth);
                NoEditTextField(value2, textfieldWidth);
            }
            GUILayout.EndHorizontal();
        }
        private static void NoEditTextFieldL2F2(string label1, string value1, string label2, string value2, 
            float labelWidth, float columnWidth)
        {
            GUILayout.BeginHorizontal();
            {
                float textfieldWidth = columnWidth/2 - labelWidth;
                
                EditorGUILayout.LabelField(label1, GUILayout.MaxWidth(labelWidth - 1));
                NoEditTextField(value1, textfieldWidth);
                
                EditorGUILayout.LabelField(label2, GUILayout.MaxWidth(labelWidth - 1));
                NoEditTextField(value2, textfieldWidth);
            }
            GUILayout.EndHorizontal();
        }
        ...
        /// <summary> </summary>
        protected void DisplayFilePartsOLD(string[] ccsa, float columnWidth)
        {
        ...
        protected void DisplayFilePartsX2(string[] ccsa, float columnWidth)
        {
            if (ccsa == null || ccsa.Length != 4) ccsa = new string[] {SC.N, SC.N, SC.N, SC.N,};

            float labelWidth = 60;
            NoEditTextFieldL1F2(EDSC.MAP_LOCATION, ccsa[2], ccsa[1], labelWidth, columnWidth);
            
            NoEditTextFieldL2F2(EDSC.MAP_COUNTRY, ccsa[0], EDSC.MAP_AREA, ccsa[3], labelWidth, columnWidth);
        }
        protected void DisplayFileParts(string[] ccsa, float columnWidth)
        {
            ...
            float labelWidth = 60;
            NoEditTextField(EDSC.MAP_LOCATION, null, ccsa[2], labelWidth, columnWidth);
            NoEditTextField(SC.S, null, ccsa[1], labelWidth, columnWidth);
            NoEditTextField(EDSC.MAP_ISO3166, null, ccsa[0], labelWidth, columnWidth);
            NoEditTextField(EDSC.MAP_AREA, null, ccsa[3], labelWidth, columnWidth);
        }
        ...
        private void OnEnable()
        {
            ...
            SceneSaveMonitor.register(this);

        }

        protected bool sceneSaved = false;
        public virtual void OnWillSaveScene()
        {
            sceneSaved = true;
        }
