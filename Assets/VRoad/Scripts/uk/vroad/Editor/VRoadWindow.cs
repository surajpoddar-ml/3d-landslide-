...
namespace uk.vroad.Editor
{
    ...
    public static class EDSC // Editor string constants, for translation (see note on Babyl, at end)
    {
        ...
        public const string MAP_ISO3166 = "ISO3166";
        public const string MAP_LOCATION = "Location";
        ...
        public const string MAPBOX_KEY_REQUIRED = "[Optional - Uses Token]";
        
        public const string MAPBOX_KEY_TT =
            "[Optional] Paste your map box token here, to download terrain elevation and aerial image tiles. " +
        ...
        public const string COPY_TO_ = "Copy To ";
        public const string REPLACE_IN_ = "Replace In ";
        public const string NO_STREAMING_TARGET = "No folder: ";

        public const string MOVED_MAPS_MSG = "Moved %d maps from vroadW to Maps";
    ...
    public static class EDSF // Editor string constants, NO translation
    {
        ...
        public const string LEGACY_VROADW = "vroadW";
        public const string MOVED_TO_MAPS = "README_MovedToMapsFolder.txt";
    ...
    public class VRoadWindow : VRoadEditorWindow
    {
        ...
        static void Init() // To call this after script edit, see forceInitOnScriptReload below 
        {
            ...
            CheckMoveVRoadStreaming();
            
            KEnv.Awake();
            
            CheckMoveLegacyVRoadW();
            CheckPipeline();
        ...
        static void CheckMoveVRoadStreaming()
        {
            try 
            {
                string vrStreamNoSlash = KC.ENV_VROAD_STREAMING_DIR.Remove(KC.ENV_VROAD_STREAMING_DIR.Length - 1);
                string asa = KC.ENV_ASSETS_DIR + KC.ENV_STREAMING_ASSETS_DIR;
                string loc1  = asa + vrStreamNoSlash;
                string loc1b = asa + vrStreamNoSlash + "Blocker";
                string loc2  = KC.ENV_ASSETS_DIR + KC.ENV_VROAD_DIR + vrStreamNoSlash;

                if ((new KDir(loc2)).Exists() && !(new KDir(loc1)).Exists() && !(new KDir(loc1b)).Exists())
                {
                    // If we are here then
                    //  - exists = NO:  loc1  - Assets/StreamingAssets/VRoadStreaming/  
                    //  - exists = NO:  loc1b - Assets/StreamingAssets/VRoadStreamingBlocker/  
                    //  - exists = YES: loc2  - Assets/VRoad/VRoadStreaming/
                    //
                    // loc1b is used only in our projects for RoadFab/RoadFabLite/RoadFabDev,
                    // to ensure that assets stay within the VRoad/ folder for packaging into Asset Store
                    // We cannot create an empty loc1 because the system will expect to find Translations there

                    // First create Assets/StreamingAssets/ if it does not exist
                    KDir asad = new KDir(asa);
                    if (!asad.Exists()) { asad.Create(); AssetDatabase.Refresh(); } 

                    // Move the whole directory at loc2 to loc1
                    string src = loc2;
                    string dst = loc1;
        ...
        static void CheckMoveLegacyVRoadW()
        {
            KDir mapsW = new KDir(KEnv.VroadWriteDir()); // Calling this creates it if it does not exist
            
            if (!mapsW.Exists()) return; // Failed to create it - file permissions?

            KDir legacy_vroadW = new KDir(mapsW.Parent(), EDSF.LEGACY_VROADW);
            if (!legacy_vroadW.Exists()) return;

            KFile movedToMaps = new KFile(legacy_vroadW, EDSF.MOVED_TO_MAPS);
            if (movedToMaps.Exists()) return; // already moved maps

            int movedCount = 0;
            String movedMapsMsg  = SC.N;
            KFile[] oldMaps = legacy_vroadW.FilesMatching(SC.SUFFIX_DOT_VROAD);
            foreach (KFile oldMap in oldMaps)
            {
                KFile newMap = new KFile(mapsW, oldMap.FileName());

                if (!newMap.Exists())
                {
                    oldMap.MoveTo(newMap);
                    movedMapsMsg += oldMap.FileName() + SC.NL;
                    movedCount++;
                }
            }
            
            KWriter.Write(movedToMaps, movedMapsMsg); // write file even if nothing moved, one-off.

            if (movedCount > 0) Debug.Log(KFormat.Sprintf(EDSC.MOVED_MAPS_MSG, movedCount));
        }
        ...
        private string streamingTargetStringShort;
        private KDir streamingTargetDir;
        ...
        void SetupReferencesInEditMode()
        {
            ...
            if (activeTab == ITAB_TRAFFIC)
            {
                ...
                if (runTraffics.Length == 1)
                {
                    runTraffic = runTraffics[0];

                    string targetAbs = KEnv.VroadReadDir();
                    streamingTargetDir = new KDir(targetAbs);
                    if (!streamingTargetDir.Exists()) streamingTargetDir.Create();
                    if (!streamingTargetDir.Exists()) streamingTargetDir = null;
                    int vroadRootIndex = targetAbs.IndexOf(KC.SLASH+KC.ENV_VROAD_STREAMING_DIR);
                    string target = targetAbs.Substring(1+vroadRootIndex);
                    if (target.Length > 32) target = SC.ELLIPSIS+target.Substring(target.Length-32);
                    streamingTargetStringShort = target;
                }
        ...
        void OnGUI()
        {
            ...
            if (IsWindowActiveInCurrentScene())
            {
                ...
                if (playStateChanged || hasSceneChanged || activeTabChanged || optionsChanged || sceneSaved)
                {
            ...
            switch (activeTab)
            {
                ...
                case ITAB_TRAFFIC:
                {
                    ...
                    GUILayout.BeginArea(AreaRect(line, nl, columnWidth), EditorStyles.helpBox);
                    {
                        ...
                        GUILayout.BeginHorizontal();
                        bool invalidFile = string.IsNullOrWhiteSpace(vroadFileName);
                        string displayedLabel = invalidFile ? EDSC.HINT_VROAD : vroadFileName;
                    ...
                    GUILayout.BeginArea(AreaRect(line, nl, columnWidth), GUIStyle.none);
                    {
                        ...
                        if (!isPlaying)
                        {
                            KFile srcFile = new KFile(vroadFilePath);
                            bool srcOK = validVRoadFileSelected && srcFile.Exists();
                            GUI.enabled = false;

                            if (!srcOK)
                            {
                                // No message or button
                            }
                            else if (streamingTargetDir == null)
                            {
                                EditorGUILayout.HelpBox(EDSC.NO_STREAMING_TARGET + KEnv.VroadReadDir(), MessageType.Warning);
                            }
                            else
                            {
                                var optHt = GUILayout.Height(helpBoxHeight);
                                GUI.enabled = true;

                                KFile destFile = new KFile(streamingTargetDir, srcFile.FileName());
                                if (destFile.Exists())
                                {
                                    if (GUILayout.Button(EDSC.REPLACE_IN_ + streamingTargetStringShort, optHt))
                                    {
                                        try
                                        {
                                            string src = srcFile.FullPath(); string dst = destFile.FullPath();
                                            FileUtil.ReplaceFile(src, dst); 
                                            Debug.Log("Replaced: " + src+ " >>> "+dst);
                                        }
                                        catch (Exception x) { Debug.LogWarning(x); }
                                    }
                                }
                                else if (GUILayout.Button(EDSC.COPY_TO_+streamingTargetStringShort, optHt))
                                {
                                    try
                                    {
                                        string src = srcFile.FullPath(); string dst = destFile.FullPath();
                                        FileUtil.CopyFileOrDirectory(src, dst);
                                        AssetDatabase.Refresh(); // Updates the Unity project view
